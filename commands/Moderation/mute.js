...
module.exports = class extends Command {
    ...
    async run(msg, [when, member, reason]) {
        ...
        if (when) {
            ...
            return msg.util.send(`${member.user.tag} got temporarily muted for ${Duration.toNow(when)}.${reason ? ` With reason of: ${reason}` : ''}`);
        }

        return msg.util.send(`${member.user.tag} got muted.${reason ? ` With reason of: ${reason}` : ''}`);
    }
